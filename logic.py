...
class DatabaseManager:
    ...
    def get_rating(self):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('''SELECT users.user_name, COUNT(winners.prize_id) as count_prize FROM winners INNER JOIN users on users.user_id=winners.user_id
                        GROUP BY winners.users_id ORDER BY count_prizes
                        LIMIT 10''')
            return cur.fetchall()
